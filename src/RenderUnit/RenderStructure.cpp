...
namespace RenderDatabase {
    ...
    const glm::vec3 MIDDLE_GRAY_COLOR = glm::vec3(0.5f, 0.5f, 0.5f);
...
void Mesh::Draw(Shader* shader, bool doingShadow)
{
...
void Model::Draw(Shader* shader, bool doingShadow)
{
    for (unsigned int i = 0; i < meshes.size(); i++)
        meshes[i].Draw(shader, doingShadow);
}
    ...
    for (unsigned int i = 0; i < mesh->mNumVertices; i++)
    {
        ...
        if (mesh->mTextureCoords[0]) // does the mesh contain texture coordinates?
        {
            ...
            if (mesh->mTangents) {
                ...
                vertex.Tangent = glm::vec3(0.0f, 0.0f, 0.0f); // 若無 tangent，初始化為零
            }
            ...
            if (mesh->mBitangents) {
                ...
                vertex.Bitangent = glm::vec3(0.0f, 0.0f, 0.0f); // 若無 bitangent，初始化為零
            }
